...
notes_misleading <- notes %>% rename(Other = misleadingOther,  Factual_Error = misleadingFactualError, Missing_Important_Context = misleadingMissingImportantContext, 
  Unverified_Claim_As_Fact = misleadingUnverifiedClaimAsFact, Outdated_Information = misleadingOutdatedInformation, Satire = misleadingSatire, 
...
ratings_summary <- ratings %>%
  group_by(noteId) %>%
  summarize(
    total_helpful = sum(helpful, na.rm = TRUE),
    total_unhelpful = sum(notHelpful, na.rm = TRUE)
  ) %>%
  mutate(
...
# 10% for test set
num_test <- floor(len * 0.1)
test_ndx <- sample(1:len, num_test, replace = FALSE)
notes_ratings_test <- notes_ratings_reg[test_ndx, ]

# 90% for validation set + training set
notes_ratings_90 <- notes_ratings_reg[-test_ndx, ]
train_ndx <- sample(1:nrow(notes_ratings_90), size = floor(0.8 * nrow(notes_ratings_90)), replace = FALSE)
notes_ratings_train <- notes_ratings_90[train_ndx, ]
notes_ratings_validate <- notes_ratings_90[-train_ndx, ]


reg_model <- glm(helpful ~ classification + trustworthySources + scale(word_count) + scale(source_followers_count) + source_verified + scale(source_friends_count) + 
  scale(age), family = binomial, data = notes_ratings_train)

coefficients_ratio <- as.data.frame(summary(reg_model)$coefficients)
coefficients_ratio$predictor <- rownames(coefficients_ratio)
ggplot(coefficients_ratio) + geom_point(aes(x = predictor, y = Estimate), color = "blue", size = 5) + 
